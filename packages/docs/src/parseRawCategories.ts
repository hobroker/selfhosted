...
import { ParsedCategory, RawCategory } from "./types";

export const parseRawCategories = (sources: RawCategory[]): ParsedCategory[] =>
  sources.map(({ category, services: _services }) => {
    ...
    const services = _services.map(({ service, path }) => {
      console.log(chalk.blueBright(chalk.bold("  Parsing service:"), chalk.italic(service)));
