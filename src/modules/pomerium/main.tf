locals {
  ...
  mounts = {
    ...
    (docker_volume.data_volume.name)   = "/data",
  }
}

data "docker_registry_image" "image" {
  name = "pomerium/pomerium:latest"
}
...
resource "docker_service" "app" {
  ...
  task_spec {
    ...
    container_spec {
      ...
      env   = merge(var.env, {
        CERTIFICATE_FILE     = "/config/certs/fullchain.pem"
        CERTIFICATE_KEY_FILE = "/config/certs/privkey.pem"
      })
